...
const PIPELINE: ApplicationStatus[] = ['applied', 'shortlisted', 'interview', 'selected', 'offer'];
...
export default function ApplicationsPage() {
  ...
  return (
    <div>
      <PageHeader title="My Applications" description="Track the status of all your applications." />
        ...
        {PIPELINE.map((stage, i) => {
          ...
          return (
            <div key={stage} className="flex items-center">
              ...
              {i < PIPELINE.length - 1 && <div className="mx-1 h-px w-8 bg-border" />}
            </div>
          );
        })}
      </div>
          ...
          <tbody className="divide-y divide-border">
            {myApps.map((app) => (
              <tr key={app.id} className="transition-colors hover:bg-muted/30">
                ...
                <td className="px-6 py-4 text-sm text-muted-foreground">{app.updatedAt}</td>
              </tr>
            ))}
            {myApps.length === 0 && (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-muted-foreground">No applications yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
