...
export default function DrivesPage() {
  ...
  const alreadyApplied = (driveId: string) => applications.some((a) => a.studentId === user?.id && a.driveId === driveId);
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map((drive, i) => (
          ...
          >
            ...
            <p className="mt-3 line-clamp-2 flex-1 text-xs text-muted-foreground">{drive.description}</p>
            <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
              <span className="rounded bg-muted px-2 py-1">💰 {drive.package}</span>
              <span className="rounded bg-muted px-2 py-1">📍 {drive.location}</span>
              <span className="rounded bg-muted px-2 py-1">📅 {drive.deadline}</span>
            </div>
            <div className="mt-3 text-xs text-muted-foreground">
              Min CGPA: {drive.eligibility.minCgpa} · Max Backlogs: {drive.eligibility.maxBacklogs}
            </div>
            <div className="mt-3 flex flex-wrap gap-1">
              {drive.eligibility.allowedDepartments.map((dept) => (
                <span key={dept} className="rounded bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent">{dept}</span>
              ))}
            </div>
            ...
            {user?.role === 'admin' && (
              <div className="mt-4 flex gap-2">
                <Button variant="outline" size="sm" className="flex-1 text-xs" onClick={(e) => { e.stopPropagation(); setSelectedDrive(drive); }}>
      ...
      {filtered.length === 0 && (
        <div className="py-16 text-center text-sm text-muted-foreground">No drives match your filters.</div>
      )}
